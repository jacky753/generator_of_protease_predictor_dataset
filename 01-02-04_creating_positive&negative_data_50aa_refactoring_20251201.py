...
def create_posi_nega_dataset(merops_code_mece, cur, error_list, df_full_aa_shorter_than_trim_len, trim_len):
    ...
    for protease in ['S01.247']:
        ...
        df_negative_pattern = pd.DataFrame(data={
            'protease_turn': [], 'merops_id': [], 'substrate_turn': [], 'uniprot_id':[], 'p1':[], 'negative_pattern':[], 'full_aa': [], 'start': [], 'end': [], 'a[k]': []
        })
            ...
            if list_subs[i][0] == list_subs[i-1][0]:
                ...
                if len(ranges) == 0:
                    ...
                    for j in range(len(ranges)):
                        ...
                        np.random.shuffle(a)
                        repeat_num = (temp_negative_pattern[1] - temp_negative_pattern[0])//50
                        for k in range(repeat_num): 
                            ...
                            df_negative_pattern.loc[f'{num_negative_data}'] = [protease_turn, protease, substrate_turn, uniprot_id, p1, full_aa[a[k]: a[k]+50], full_aa, ranges[j][0], ranges[j][1], a[k]]
                            num_negative_data += 1
